...
def task(relationship_csv: str) -> float:
     # Разделяем строку по символам новой строки
     rows = relationship_csv.strip().split("\n")
     ...
     # считаем энтропию
     entropy = 0
     LENGTH_MATRIX = len(matrix) - 1
     for row in matrix:
         for value in row:
             if float(value) / (LENGTH_MATRIX) != 0:
                  entropy += value / (LENGTH_MATRIX) * math.log2(value / (LENGTH_MATRIX))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "task":
          relationship_csv = sys.argv[2] 
          result = task(relationship_csv)
          print("entropy=" + str(result))
        else:  
          input_string = sys.argv[1]
          relationship_csv = main(input_string)
          result = task(relationship_csv)
          print("entropy=" + str(result))
    else:
